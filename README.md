...
An exploratory analysis of restaurant sales and customer satisfaction performance using data captured through orders on a food delivery app. The goal is to draw insights to support the app management team by discovering factos that influence higher restaurant sales and better customer ratings through comparing key dimensions such as restaurants, zones, cuisines and key metrics such as delivery time and quality, and customer food ratings.

## Datasets

The data represents 500 orders placed across 20 restaurants on a food delivery platform over the course of a single day (01/01/2022). It consists of two relational datasets, one detailing orders, and the other containing restaurant information. The datasets are linked via the 'restaurant_id' key.
...
To build on this, using a dataset across a greater time frame would allow for more data to be collected leading to more accurate representation of business activity, enabling more valuable insight to be uncovered.
